...
class RecommendationSystem:
    ...
    """
            ...
            for a, b in itertools.combinations(artists, 2):
                ...
                if pair not in compared_pairs:
                    ...
                    if similarity > 0 and b['b_name'] not in artist_names:  # Chequear que la similitud sea mayor a 0 y que el artista no haya sido ya agregado
    ...
    def calculate_similarity(self, a, b):
        style_sim = self.jaccard_similarity(set(a['a_style']), set(b['b_style']))
        mood_sim = self.jaccard_similarity(set(a['a_mood']), set(b['b_mood']))
        genre_sim = self.jaccard_similarity(set(a['a_genre']), set(b['b_genre']))
        location_sim = self.jaccard_similarity(set(a['a_location']), set(b['b_location']))
        popularSong_sim = self.jaccard_similarity(set(a['a_popularSong']), set(b['b_popularSong']))
        influencedBy_sim = self.jaccard_similarity(set(a['a_influencedBy']), set(b['b_influencedBy']))
        collaboratedWith_sim = self.jaccard_similarity(set(a['a_collaboratedWith']), set(b['b_collaboratedWith']))
        decade_sim = self.jaccard_similarity(set(a['a_decade']), set(b['b_decade']))
